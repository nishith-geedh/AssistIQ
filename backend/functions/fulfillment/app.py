...
# --- Core helper: finalize/fulfill an intent ---
...
def lambda_handler(event, context):
    ...
    # --- Handle Lex built-in confirmationState globally ---
    ...
    # --- Global confirm slot handler (applies to ALL intents with slot `confirm`) ---
    ...
    if confirm_slot and confirm_slot.get("value"):
        ...
        if interpreted in negatives:
            ...
            log_interaction(user_text, intent_name or "UnknownIntent", 1.0, session_id, reply)
            clear_session_state(session_id)
            # IMPORTANT: close conversation so Lex doesn’t continue prompting
            return {
                "sessionState": {
                    "dialogAction": {"type": "Close"},
                    "intent": {"name": intent_name or "FallbackIntent", "state": "Fulfilled"}
                },
                "messages": [{"contentType": "PlainText", "content": reply}]
            }

        # If not clear, re-prompt
    ...
    # --- Legacy session-based confirmation ---
